...
class Experiment(object):
    ...
    def meta_loss(self, datasets):
        ...
        for i in range(0,9):
            inputs = self.valid_set_meta.X[i*220+125:i*220 + 130]
            targets =self.valid_set_meta.y[i*220+125:i*220 + 130]
            ...
            self.model.eval()
            inputs = self.valid_set_meta.X[i*220 + 130:i*220 + 180]
            targets =self.valid_set_meta.y[i*220 + 130:i*220 + 180]
            ...
            # # inputs = np.concatenate((inputs, X_aug),axis=0)
            # # targets = np.concatenate((targets, Y_aug),axis=0)
            # inputs = X_aug
            # targets = Y_aug
    ...
    def train_batch(self, inputs, targets):
        ...
        #         X_aug[i,j,rand_int:rand_int+100] = 0
        # inputs = np.concatenate((inputs, X_aug),axis=0)
        # targets = np.concatenate((targets, Y_aug),axis=0)
        ## 
